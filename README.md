...
| <a name="input_agent_linux_parameters"></a> [agent\_linux\_parameters](#input\_agent\_linux\_parameters) | Linux-specific parameters for the Datadog Agent container.<br/>Selected parameters are supported by AWS Fargate.<br/>See: https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_LinuxParameters.html | <pre>object({<br/>    capabilities = optional(object({<br/>      add  = optional(list(string))<br/>      drop = optional(list(string))<br/>    }))<br/>    devices = optional(list(object({<br/>      hostPath      = string<br/>      containerPath = string<br/>      permissions   = optional(list(string))<br/>    })))<br/>    initProcessEnabled = optional(bool)<br/>  })</pre> | `{}` | no |
...
| <a name="input_attributes"></a> [attributes](#input\_attributes) | List of attributes to include in resource names. Overrides attributes set in 'context' variable. | `list(string)` | `[]` | no |
...
| <a name="input_enable_cloud_network_monitoring"></a> [enable\_cloud\_network\_monitoring](#input\_enable\_cloud\_network\_monitoring) | Enable Cloud Network Monitoring (CNM) for enhanced network visibility.<br/>Requires Datadog Agent version 7.32.0+ and appropriate permissions.<br/>See: https://docs.datadoghq.com/network_monitoring/cloud_network_monitoring/setup | `bool` | `false` | no |
...
| <a name="output_dd_api_key_kms_key_id"></a> [dd\_api\_key\_kms\_key\_id](#output\_dd\_api\_key\_kms\_key\_id) | The KMS key ID used to encrypt the Datadog API key secret, if configured. |
| <a name="output_dd_api_key_secret_arn"></a> [dd\_api\_key\_secret\_arn](#output\_dd\_api\_key\_secret\_arn) | The ARN of the Secrets Manager secret containing the Datadog API key, if configured. |
