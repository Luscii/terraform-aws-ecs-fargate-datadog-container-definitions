...
| <a name="input_ecr_registry_url"></a> [ecr\_registry\_url](#input\_ecr\_registry\_url) | ECR registry URL for pull-through cache (e.g., '123456789012.dkr.ecr.eu-west-1.amazonaws.com'). When provided together with pull\_cache\_prefix in image configuration, container images will be pulled through ECR instead of directly from the source registry. Leave null to pull directly from source registries (Docker Hub, etc.). | `string` | `null` | no |
