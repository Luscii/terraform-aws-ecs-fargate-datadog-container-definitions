...
check "validate_custom_log_parsers_s3_bucket" {
  assert {
    condition     = length(var.log_config_parsers) == 0 || var.s3_config_bucket_name != null
    error_message = "When log_config_parsers is configured, s3_config_bucket_name must be set to store the custom FluentBit configuration files."
  }
}

check "validate_custom_log_parsers_init_image" {
  assert {
    condition = length(var.log_config_parsers) == 0 || (
      can(regex("^init-", var.log_router_image_tag)) ||
      can(regex("^(stable|latest|[0-9]+\\.[0-9]+\\.?[0-9]*)$", var.log_router_image_tag))
    )
    error_message = "When log_config_parsers is configured, log_router_image_tag must either be an init-tagged image (e.g., 'init-3.2.0') or a standard version tag that will be automatically prefixed with 'init-' by the module (e.g., '3.2.0' becomes 'init-3.2.0'). The module automatically handles the init prefix for you."
  }
}
